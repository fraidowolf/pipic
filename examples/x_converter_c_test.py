...
sim.add_particles(
    ...
    mass=electron_mass,
    temperature=temperature,
    density=density_callback1.address,
)

# ------------------------------adding extension---------------------------------
extension_handler = x_converter_c.handler(
    location=-L / 4 - L / 32, thickness=L / 16, typeTo=sim.get_type_index("electron")
)
sim.add_handler(name=x_converter_c.name, subject="electron1", handler=extension_handler)
