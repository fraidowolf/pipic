...
temperature = (2 / 3) * 1e-6 * consts.electron_mass * consts.light_velocity**2
density = 1e18
...
field_amplitude = 0.001 * 4 * np.pi * (xmax - xmin) * consts.electron_charge * density
nx = 32  # number of cells
iterations_period = 8
time_step = plasma_period / iterations_period
fig_label = "ec2"
...
mc2 = consts.electron_mass * consts.light_velocity**2
...
def getKineticEn(sim):
    kineticEnergy[0] = 0
    sim.particle_loop(
        name="electron", handler=kineticEn_cb.address, data_double=pipic.addressof(kineticEnergy)
    )
...
factor = (
    ((sim.xmax - sim.xmin) / sim.nx)
    * ((sim.ymax - sim.ymin) / sim.ny)
    * ((sim.zmax - sim.zmin) / sim.nz)
    / (8 * np.pi)
)
field_energy = np.zeros((1,), dtype=np.double)


@cfunc(types.field_loop_callback)
def fieldEn_cb(ind, r, E, B, data_double, data_int):
    data_double[0] += factor * (
        E[0] ** 2 + E[1] ** 2 + E[2] ** 2 + B[0] ** 2 + B[1] ** 2 + B[2] ** 2
    )


def getFieldEnergy(sim):
    field_energy[0] = 0
    sim.field_loop(handler=fieldEn_cb.address, data_double=pipic.addressof(field_energy))
...
name = fig_label + "_" + str(iterations_period)
