...
def pulse(eta, phi):
    return (
        (abs(eta - 2 * np.pi) < 2 * np.pi)
        * np.sin(eta / 4) ** 3
        * (np.cos(eta / 4) * np.sin(eta + phi) + np.sin(eta / 4) * np.cos(eta + phi))
    )
...
def field_callback(ind, r, E, B, data_double, data_int):
    if data_int[0] == 0 or ymax - r[1] < boundarySize:
        ...
        lCoord = (
            np.sin(incidenceAngle) * r[0]
            - np.cos(incidenceAngle) * r[1]
        ...
        f_2 = (
            fieldAmplitude * tShape * pulse(2 * np.pi * lCoord / wavelength + 4 * np.pi, np.pi / 2)
        )
...
sim.add_particles(
    name="electron",
    number=int(nx * ny * 0.25 * particlesPerCell),
