...
s = int(4*Lx*Tr/timestep) #2 #int((Tr*fwhm_duration_laser_pulse_s/2)/timestep) #3000*10 # number of iterations 
checkpoint = s//10   
print(s,checkpoint)
...
def initiate_field_callback(ind, r, E, B, data_double, data_int):
    ...
    if data_int[0] == 0:       
        ...
        # x-polarized 
        E[0] = gp #*np.cos(omega_r*0)       
        B[1] = gp #*np.cos(omega_r*(timestep/2))        
...
def load_fields():
    ...
    sim.field_loop(handler=get_field_Bx.address, data_double=pipic.addressof(Bx))
    sim.field_loop(handler=get_field_By.address, data_double=pipic.addressof(By))
    sim.field_loop(handler=get_field_Bz.address, data_double=pipic.addressof(Bz))
