...
# conversions
Nr = m*omega_r**2/(np.pi*4*e**2) # (2.np.pi)**2/cm³
...
#s = 4*50000 #3000*10 # number of iterations 
checkpoint = 100   



#---------------------setting solver and simulation region----------------------
sim=pipic.init(solver='fourier_boris',nx=nx,ny=ny,nz=nz,xmin=xmin,xmax=xmax,ymin=ymin,ymax=ymax,zmin=zmin,zmax=zmax)
sim.en_corr_type(2)
...
def get_phase_space(r, p, w, id, data_double, data_int):   
    ...
    gamma = np.sqrt(1+(p[0]**2 + p[1]**2 + p[2]**2)/(m*consts.light_velocity)**2)
    ekin = (gamma - 1)*m*consts.light_velocity**2/1.60218e-6 #MeV
    ip = int(ps.shape[0]*(ekin - pmin)/(pmax - pmin))
